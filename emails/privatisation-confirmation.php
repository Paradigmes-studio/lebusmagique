...
if ($type === 'entreprise') {
    $intro = 'Nous avons bien reçu votre demande de privatisation pour le compte de ' . esc_html($organisation) . '.';
    $salutation = 'Bonjour,';
} elseif ($type === 'association') {
    $intro = 'Nous avons bien reçu la demande de privatisation de ' . esc_html($organisation) . ' et sommes ravis de soutenir votre projet.';
    $salutation = 'Bonjour,';
} else {
    $intro = 'Nous avons bien reçu votre demande de privatisation et nous réjouissons de vous accueillir à bord !';
...
<body style="font-family: Helvetica, Arial, sans-serif; font-size: 14px; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="padding: 25px 0;">
        ...
        <p><?php echo $intro; ?></p>
        <p>Vous trouverez ci-joint votre devis n° <strong><?php echo esc_html($devis_number); ?></strong> pour la privatisation de la péniche.</p>
        ...
        <div style="background: #f8f4e8; padding: 15px; margin: 20px 0; border-radius: 5px;">
            ...
            <p><strong>Montant TTC :</strong> <?php echo number_format((float)$montant_ttc, 2, ',', ' '); ?> €</p>
        </div>
